...
const { buildCs } = require('./types/c-sharp.service');
const { buildPhp } = require('./types/php.service');
const { buildJava } = require('./types/java.service');
const defaults = {
...
function exportModel(argv, sequelizePath) {
    ...
    switch (extension) {
        ...
        case 'py':
            ...
            break;
        case "cs":
            jsonString = buildCs(model, modelname);
            break;
        case "php":
            jsonString = buildPhp(model, modelname);
            break;
        case 'java':
            jsonString = buildJava(model, modelname)
            break;
        default:
