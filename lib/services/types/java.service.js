const { template, uniq, camelCase, forEach, upperFirst, map } = require("lodash");
...
function buildJava(model, modelName) {
    ...
    const constructorParams = map(properties, item=>`${item.type} ${item.name}`)
    const props = { className: modelName, properties, packages: uniq(imports), constructorParams: constructorParams.join(',') };
    const compile = template(templateFile);
    return compile(props).replace(/&lt;/g, "<").replace(/&gt;/g, ">").replace(/&quot;/g, '"');
}
